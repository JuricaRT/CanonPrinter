\chapter{Implementacija i korisničko sučelje}
		...
		\section{Upute za puštanje u pogon}
			...
			\noindent{Svaki par MongoDB baze podataka i korisnika s kojim se spajamo na bazu podataka ima svoj URL oblika "mongodb+srv://$\langle$korisnik$\rangle$:$\langle$lozinka$\rangle$@$\langle$baza podataka$\rangle$.$\langle$cluster ID$\rangle$.mongodb.net/<opcije>" koji se naziva \textit{database connection string}. URL aplikacija koristi da bi se spojila na bazu podataka. URL se dobiva odlaskom na Deployment $\rightarrow$ Database $\rightarrow$ $\langle$baza podataka$\rangle$ $\rightarrow$ Connect $\rightarrow$ Drivers gdje će se prikazati URL koji se može kopirati.} \\
